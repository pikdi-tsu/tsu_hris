...
@section('title', $title ?? 'Road Map Dosen per Program Studi')
...
@section('css')
    <style>
        ...
        .tsu-matrix-table thead th.th-target {
            background: #ecfdf5;
            color: #047857;
            border-color: #a7f3d0;
        }

        .tsu-matrix-table tbody td {
            vertical-align: middle;
            font-size: 0.85rem;
        ...
        .tsu-matrix-table tbody tr:hover td {
...
@section('content')
    {{-- TSU Page Header --}}
    <x-tsu-page-header
        :title="$title ?? 'Road Map Dosen per Program Studi'"
        subtitle="Rencana Pengembangan Kualifikasi Doktor (S3) &amp; Sertifikasi (2026 - 2030)"
    ...
    >
        <x-slot name="actions">
            ...
            {{-- Tombol Tambah Dosen Baru --}}
            <button type="button" class="btn btn-sm tsu-btn-primary-action" data-toggle="modal" data-target="#modalTambahDosen">
    ...
    <section class="content">
        <div class="container-fluid">
            ...
            {{-- Filter Bar Program Studi & Periode --}}
            <div class="tsu-filter-card">
                <form method="GET" action="{{ route('admin.pengembangan-sdm.dosen') }}" id="formProdi" class="row align-items-center">
                    <div class="col-lg-5 col-md-6 col-12 mb-2 mb-md-0">
                        <span class="tsu-filter-label">Pilih Program Studi Homebase:</span>
                    ...
                    @if(count($periodeList) > 1)
                        <div class="col-lg-4 col-md-4 col-12 mb-2 mb-md-0">
                            ...
                            <select name="periode_id" class="form-control select2" style="width: 100%;" onchange="$('#formProdi').submit();">
            ...
            {{-- 4 Stat Cards: Metrics Ringkasan Prodi --}}
            <div class="tsu-stat-grid-roadmap">
                {{-- Total Dosen --}}
                <div class="tsu-stat-card tsu-stat-card--total">
                    <div class="tsu-stat-card__icon">
                        <i class="fas fa-chalkboard-teacher"></i>
                    </div>
                    <div class="tsu-stat-card__title">Total Dosen Prodi</div>
                    <div class="tsu-stat-card__value">{{ $pesertas->count() }} <span style="font-size: 1rem; font-weight: 600;">Orang</span></div>
                    <div class="tsu-stat-card__subtext">{{ $unit->nama_unit ?? 'Program Studi' }}</div>
                </div>

                {{-- S3 Saat Ini (2026) --}}
                <div class="tsu-stat-card tsu-stat-card--s3">
                    <div class="tsu-stat-card__icon">
                        <i class="fas fa-user-graduate"></i>
                    </div>
                    <div class="tsu-stat-card__title">S3 Saat Ini (2026)</div>
                    <div class="tsu-stat-card__value">{{ $prodi_stats[2026]['s3'] ?? 0 }} <span style="font-size: 1rem; font-weight: 600;">Orang</span></div>
                    <div class="tsu-stat-card__subtext">Proporsi: {{ $prodi_stats[2026]['persen_s3'] ?? 0 }}% dari total dosen</div>
                </div>

                {{-- Target S3 (2030) --}}
                <div class="tsu-stat-card tsu-stat-card--target">
                    <div class="tsu-stat-card__icon">
                        <i class="fas fa-flag-checkered"></i>
                    </div>
                    <div class="tsu-stat-card__title">Target S3 (2030)</div>
                    <div class="tsu-stat-card__value">{{ $prodi_stats[2030]['s3'] ?? 0 }} <span style="font-size: 1rem; font-weight: 600;">Orang</span></div>
                    <div class="tsu-stat-card__subtext">Target: {{ $prodi_stats[2030]['persen_s3'] ?? 0 }}% pada akhir periode</div>
                </div>

                {{-- Sedang Studi (SS) --}}
                <div class="tsu-stat-card tsu-stat-card--ss">
                    <div class="tsu-stat-card__icon">
                        <i class="fas fa-book-reader"></i>
                    </div>
                    <div class="tsu-stat-card__title">Sedang Studi (SS)</div>
                    <div class="tsu-stat-card__value">{{ $prodi_stats[2026]['ss'] ?? 0 }} <span style="font-size: 1rem; font-weight: 600;">Orang</span></div>
                    <div class="tsu-stat-card__subtext">Aktif studi lanjut tahun 2026</div>
                </div>
            </div>
            ...
            <div class="modal-content">
                <div class="modal-header tsu-modal-header">
                    <h5 class="modal-title font-weight-bold">
                        <i class="fas fa-user-plus mr-2"></i> Tambah Rencana Dosen Baru
                    </h5>
                ...
                <form method="POST" action="{{ route('admin.pengembangan-sdm.add-dosen-baru') }}">
                    ...
                    <div class="modal-body p-4">
                        <div class="row">
                            <div class="col-md-6 form-group">
                                <label class="font-weight-bold small text-dark">Nama Dosen / Proyeksi Rekrutmen <span class="text-danger">*</span></label>
                                <input type="text" name="nama" class="form-control" required placeholder="Contoh: Dosen Baru Informatika 1 / Nama Kandidat">
                            </div>
                            <div class="col-md-6 form-group">
                                <label class="font-weight-bold small text-dark">Bidang Keilmuan Homebase</label>
                        ...
                        <div class="row">
                            <div class="col-md-6 form-group">
                                <label class="font-weight-bold small text-dark">Rencana Lokasi Studi S3</label>
                            ...
                            <div class="col-md-6 form-group">
                                <label class="font-weight-bold small text-dark">Tahun Masuk Studi S3</label>
                                <input type="number" name="tahun_mulai" class="form-control" value="2026" min="2026" max="2030">
                            </div>
                        </div>

                        <div class="row">
                            <div class="col-md-4 form-group">
                                <label class="font-weight-bold small text-dark">Pendidikan S1</label>
                                <input type="text" name="pendidikan_s1" class="form-control" placeholder="Contoh: S1 Teknik Informatika">
                            </div>
                            <div class="col-md-4 form-group">
                                <label class="font-weight-bold small text-dark">Pendidikan S2</label>
                                <input type="text" name="pendidikan_s2" class="form-control" placeholder="Contoh: S2 Ilmu Komputer">
                            </div>
                            <div class="col-md-4 form-group">
                                <label class="font-weight-bold small text-dark">Pendidikan S3</label>
                    ...
                    <div class="modal-footer bg-light">
                        ...
                        <button type="submit" class="btn tsu-btn-primary-action">
                            <i class="fas fa-save mr-1"></i> Simpan Dosen
                        </button>
                    </div>
                </form>
            </div>
    ...
    <div class="modal fade" id="modalSertifikasi" tabindex="-1" role="dialog" aria-hidden="true">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header tsu-modal-header">
                    <h5 class="modal-title font-weight-bold">
                        <i class="fas fa-certificate mr-2"></i> Tambah Sertifikasi Dosen
                    </h5>
                    <button type="button" class="close" data-dismiss="modal">&times;</button>
                </div>
                <div class="modal-body p-4">
                    <div class="alert alert-light border mb-3 py-2 px-3">
                        <strong class="text-dark small d-block mb-1">Target Dosen:</strong>
...
<script>
    ...
    function updateTimeline(element) {
        ...
        // Update visual pill classes
        el.removeClass('status-s2 status-s2-plus status-s3');
        if (status.indexOf('S3') !== -1 && status.indexOf('+') === -1) {
            el.addClass('status-s3');
        } else if (status.indexOf('+') !== -1) {
            el.addClass('status-s2-plus');
        } else {
            el.addClass('status-s2');
        }
        ...
        $.ajax({
            ...
            error: function(err) {
                Swal.fire({
                    icon: 'error',
                    title: 'Gagal',
                    text: 'Gagal memperbarui status timeline'
                });
            }
        });
    }

    function updateLokasi(pesertaId, lokasi) {
        $.ajax({
            url: "{{ route('admin.pengembangan-sdm.update-lokasi') }}",
            type: "POST",
            data: {
                _token: "{{ csrf_token() }}",
                peserta_id: pesertaId,
            ...
            success: function(res) {
                if (res.success) {
                    ...
                    Toast.fire({
                        icon: 'success',
                        title: 'Lokasi studi berhasil diubah'
                    });
                    location.reload();
                }
            },
            error: function(err) {
                Swal.fire('Error', 'Gagal memperbarui lokasi studi', 'error');
            }
        });
    }
    ...
    function removeSertifikasi(pesertaId, sertifikasiId) {
        Swal.fire({
            ...
            text: 'Apakah Anda yakin ingin menghapus sertifikasi ini dari dosen?',
            icon: 'warning',
