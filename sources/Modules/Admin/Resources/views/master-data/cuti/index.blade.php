...
@section('content')
    <x-tsu-page-header
        :title="$title ?? 'Data Master Cuti'"
        subtitle="Kelola master kategori cuti, kuota durasi maksimal, serta aturan pemotongan cuti tahunan"
        ...
        <x-slot name="actions">
            ...
            @can('admin:master-cuti:create')
                <button type="button" class="btn btn-sm tsu-btn-primary-action btn-modal ml-2"
                    ...
                    <i class="fas fa-plus mr-1"></i> Tambah Master Cuti
                </button>
            @endcan
        </x-slot>
    </x-tsu-page-header>
    ...
    <section class="content">
        <div class="container-fluid">
            {{-- 4 Stat Cards --}}
            <div class="tsu-stat-grid-cuti">
                {{-- Total Master Cuti --}}
                <div class="tsu-stat-card tsu-stat-card--total">
                    <i class="fas fa-calendar-check tsu-stat-card__icon"></i>
                    <div>
                        <div class="tsu-stat-card__title">Total Master Cuti</div>
                    ...
                    <div class="tsu-stat-card__subtext">
                        Kategori Cuti Terdaftar di TSU HRIS
                    </div>
                </div>
                ...
                <div class="tsu-stat-card tsu-stat-card--nonaktif">
                    ...
                    <div class="tsu-stat-card__subtext">
                        Kategori Cuti Dinonaktifkan
                    </div>
                </div>

                {{-- Maksimal Durasi --}}
                <div class="tsu-stat-card tsu-stat-card--durasi">
                    <i class="far fa-clock tsu-stat-card__icon"></i>
                    <div>
                        <div class="tsu-stat-card__title">Maksimal Durasi</div>
                        <div class="tsu-stat-card__value">
                            {{ number_format($stats['max_durasi'] ?? 0) }}
                            <span class="tsu-stat-card__unit">Hari</span>
                        </div>
                    </div>
                    <div class="tsu-stat-card__subtext">
                        Durasi Tertinggi (Rata-rata: {{ $stats['avg_durasi'] ?? 0 }} Hari)
                    </div>
                </div>
            </div>
            ...
            <div class="card tsu-card">
                <div class="tsu-card__header d-flex align-items-center justify-content-between flex-wrap gap-2">
                    <div>
                        <h3 class="tsu-card__title">
                            <i class="fas fa-list-alt text-primary mr-2"></i>Daftar Master Jenis Cuti Pegawai
                        </h3>
                        <p class="text-muted text-xs mb-0 mt-1">
                            Seluruh aturan kuota, syarat hari minimal, serta parameter pemotongan hak cuti tahunan
                        </p>
                    </div>
                </div>
                ...
                <div class="card-body p-3">
                    <div class="table-responsive">
                        <table id="table-cuti" class="table tsu-table-modern table-hover w-100">
                            <thead>
                                <tr>
                                    <th width="4%" class="text-center">No</th>
                                    <th width="26%">Jenis Cuti</th>
                                    <th width="16%" class="text-center">Kategori Cuti</th>
                                    <th width="12%" class="text-center">Durasi Maks</th>
                                    <th width="20%">Pengaruh Kuota &amp; Akses</th>
                                    <th width="8%" class="text-center">Min. Hari</th>
    ...
    <div class="modal fade" id="modal-cuti" tabindex="-1" role="dialog" aria-hidden="true">
        <div class="modal-dialog modal-md modal-dialog-centered" role="document">
            <div class="modal-content" id="modal-cuti-content" style="border-radius: 12px; overflow: hidden; border: none; box-shadow: 0 10px 30px rgba(0,0,0,0.15);">
                {{-- Loading State --}}
...
@section('script')
    ...
    <script>
        $(document).ready(function() {
            var dtLanguage = {
                search: "_INPUT_",
                searchPlaceholder: "Cari jenis cuti, kategori...",
            ...
            var table = $('#table-cuti').DataTable({
                ...
                columns: [
                    ...
                    { data: 'kategori_badge', name: 'kategori_cuti', className: 'text-center' },
                    { data: 'durasicuti', name: 'durasicuti', className: 'text-center' },
                    { data: 'aturan_badge', name: 'memotong_kuota' },
                    { data: 'minimalhari', name: 'minimalhari', className: 'text-center' },
            ...
            $('body').on('submit', '#modal-cuti form', function(e) {
                e.preventDefault();
                var form = $(this);
            ...
            $('body').on('click', '.btn-delete', function(e) {
                ...
                Swal.fire({
                    ...
                    html: "Anda akan menghapus kategori cuti: <br><strong>" + rowName + "</strong>.<br><small class='text-danger font-weight-bold'>Data yang dihapus tidak dapat dipulihkan!</small>",
                    icon: 'warning',
